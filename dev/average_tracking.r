...
votes_min_max <- 
    ...
    group_by(year) %>%
    nest() %>%
    mutate(votemodel = map(data, ~lm(votesreceived ~ daysuntilelection, data = .x))) %>%
    mutate(votes = map(votemodel, ~broom::augment(.x, newdata = tibble(daysuntilelection = 1:30)))) %>%
    unnest(votes) %>%
    select(year, daysuntilelection, votesmodeled = .fitted) %>%
    group_by(daysuntilelection) %>%
    summarize(vote_max = max(votesmodeled), vote_min = min(votesmodeled), .groups = "drop") %>% 
...
votes_this_year %>%
    ...
    geom_ribbon(data = votes_min_max,
                aes(y = NULL,
                    ymin = vote_min,
                    ymax = vote_max),
                    alpha = .2) + 
